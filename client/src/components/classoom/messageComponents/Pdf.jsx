...
function Pdf({ pdf}) {
  ...
  return (
    <div className="flex flex-col w-full max-w-[700px] leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700">
            {pdf.slice(0, showAllPdf ? pdf.length : 3).map((pdf, index) => (
      ...
      <div className="flex justify-between items-center mt-4">
        <button onClick={showAllPdf ? handleHideAllPdf : handleShowAllPdf} className="text-sm text-blue-700 dark:text-blue-500 font-medium inline-flex items-center hover:underline">
          {showAllPdf ? 'Hide all' : 'Show all'}
        </button>
      </div>
    </div>
  );
